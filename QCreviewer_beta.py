...
──────────
FILE MENU
  • New "File" menu (left of "View") with a single item:
    "Change file paths…" (Ctrl+,)
  • Opens the Path Setup dialog non-destructively over the main window.
  • On confirm the diff between old and new config determines what happens:
      – Options only changed (save_gen / show_seg_derived): applied live,
        no reload.
      – CSV name only changed: in-memory results written to new CSV path,
        old CSV left untouched.  No case reload.
      – Data folder or any file name changed: full reinitialisation —
        auto-saves current case, stops all background threads, clears
        case list / cache / napari layers, reloads results from new CSV,
        and starts a fresh case scan.

REINIT ROBUSTNESS
  • Generation counter (_reinit_gen) invalidates stale prefetch-thread
    results so they are silently discarded after a reinit.
  • Current config stored as self._current_config (AppConfig) so diffs
    are always computed from the live state.
  • Napari layers are explicitly cleared on full reinit to prevent
    shape-mismatch errors when the new dataset has different geometry.

(All v12h improvements retained.)
"""
import os
import sys
import bisect
